...
class EDMD:
    def __init__(self, data: pd.DataFrame, dim: int, dict: observables_dict):
        """Initializes the Extended Dynamic Mode class object with
            training data, dictionary of observables and data dimensions
        ...
        Args:
            ...
            dict (observables_dict): Dictionary of observables to be applied to the data
                                    for the calculation of hte Koopman operator approximation matrix
        """
    ...
    def segregate_xy(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """Breaks one dataframe into two, first containing the independent variable
            and the second containing the dependent variable

        Returns:
            Tuple[pd.DataFrame, pd.DataFrame]: The independent and dependent variable in separate dataframes.
                both dataframes have ID as their first column and have a column structure as
    ...
    def build_eigenfunction(self, data: pd.DataFrame) -> np.array:
        """Calculates the all the eigenfunctions stacked as a matrix using $\Psi(x) V$
        where $\Psi(x)$ is the matrix from the dictionary of observables
    ...
    def predict(self, initial_value_df: pd.DataFrame, t_range: np.array) -> pd.DataFrame:
        ...
        Args:
            ...
            ID   time   Var_1   Var_2   ...   Var_n
            t_range (np.array): the time steps as integers

        Returns:
            pd.DataFrame: Resulting prediction
        """
        # initial_value_df.copy(deep=True)
        ...
        for id in tqdm(initial_value_df.ID.unique()):
            ...
            for t in t_range[1:]:
                ...
                prediction = pd.DataFrame(np.insert(prediction_mat, 0, prediction[['ID', 'time']].iloc[0]).real,
                                          columns=list(initial_value_df.columns))
